...
import { DAYS, DAY_SHORT, DayOfWeek, TimetableEntry, LessonSlot } from '@/types/timetable';
import { mockLessonSlots, mockTimetableEntries, getSubjectById, getTeacherById, getClassroomById } from '@/data/mockData';
...
export function WeeklyGrid({
  ...
  lessonSlots = mockLessonSlots,
  filterGroupId,
  filterTeacherId,
}: WeeklyGridProps) {
  // Get unique time rows
  ...
  // Build lookup: day+timeKey → entry
  const entryMap = useMemo(() => {
    ...
    for (const entry of filtered) {
      ...
      if (slot) {
        map.set(`${slot.day_of_week}-${slot.start_time}-${slot.end_time}`, entry);
      }
    }
    return map;
      ...
      <div className="min-w-[800px]">
        ...
        <div className="grid grid-cols-[100px_repeat(6,1fr)] gap-1 mb-1">
          ...
          {DAYS.map(day => (
            <div
              key={day}
              className="p-3 text-center text-xs font-semibold text-muted-foreground uppercase tracking-wider bg-card rounded-lg"
            >
        ...
        {timeSlots.map(ts => {
          ...
          return (
            <div key={`${ts.start}-${ts.end}`} className="grid grid-cols-[100px_repeat(6,1fr)] gap-1 mb-1">
              {/* Time label */}
              <div className={cn(
                "flex flex-col justify-center px-3 py-2 text-xs font-medium",
                isBreak ? "text-accent" : "text-muted-foreground"
              )}>
                <span>{ts.start}</span>
              ...
              {DAYS.map(day => {
                ...
                if (!entry) {
                  return (
                    <div
                      key={key}
                      className="bg-card/50 rounded-lg border border-border/50 min-h-[60px] flex items-center justify-center hover:bg-muted/50 transition-colors cursor-pointer"
                    >
                ...
                return (
                  <div
                    key={key}
                    className="rounded-lg min-h-[60px] p-2 flex flex-col justify-between text-white cursor-pointer hover:scale-[1.02] transition-transform shadow-sm"
